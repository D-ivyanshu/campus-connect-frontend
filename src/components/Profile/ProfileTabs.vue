<template>
  <div class="flex space-x-5 p-4 sm:ml-2 sm:justify-center md:justify-end">
    <button v-for="(tab, index) in props.tabs" :key="index" @click="selectTab(index)">
      {{ tab }}
    </button>
    ...
    <button @click="selectTab(4)">
      <div class="flex items-center justify-center p-1">
        ...
        >
          <path
            fill="currentColor"
            d="M18.512 10.077c0 .739-.625 1.338-1.396 1.338c-.77 0-1.395-.6-1.395-1.338c0-.739.625-1.337 1.395-1.337s1.396.598 1.396 1.337"
          ></path>
          <path
            fill="currentColor"
            fill-rule="evenodd"
          ...
          <path
            fill="currentColor"
            d="M17.086 2.61c-.86-.11-1.954-.11-3.319-.11h-3.09c-1.364 0-2.459 0-3.319.11c-.89.115-1.632.358-2.221.92a2.9 2.9 0 0 0-.724 1.12c.504-.23 1.074-.366 1.714-.45c1.084-.14 2.47-.14 4.22-.14h3.914c1.75 0 3.135 0 4.22.14c.558.073 1.064.186 1.519.366a2.875 2.875 0 0 0-.692-1.035c-.589-.563-1.331-.806-2.222-.92"
          ></path>
        </svg>
        Gallery
      </div>
